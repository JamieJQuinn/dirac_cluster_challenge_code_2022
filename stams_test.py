...
def test_conjugate_gradient_dirichlet():
    nx = 16
    ny = 31
    lx = 1.0
    ly = 1.0
    ...
    b = np.zeros_like(u)
    # hack to ensure constand part of bcs are on RHS of matrix equation
    b[1, :] = -2.0*bcs[0]/dx**2
    b[-2,:] = -2.0*bcs[2]/dx**2
    b[:,-2] = -2.0*bcs[1]/dy**2 
    b[:, 1] = -2.0*bcs[3]/dy**2 
...
def test_conjugate_gradient_von_neumann():
    nx = 16
    ny = 31
    lx = 1.0
    ly = 1.0
    ...
    u = np.sin(2*np.pi*X)*np.sin(2*np.pi*Y) + 1.0
    # u = np.zeros((nx+2, ny+2))
    ...
    solve(nabla2, u, b, temp1, temp2, nx, ny, set_von_neumann_bcs, max_iterations=100)
...
def main():
    # test_advection()
    # test_dirichlet()
    # test_conjugate_gradient_dirichlet()
    test_conjugate_gradient_von_neumann()
